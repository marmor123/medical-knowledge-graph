...
def test_mock_ingestion_pipeline():
    """Verify the end-to-end pipeline in mock mode."""
    pdf_path = "Pocket Medicine 9th Edition 2026.pdf"
    output_file = "tests/test_chunks.json"
    ...
    assert len(data) == 1
    assert data[0]["source_file"] == os.path.basename(pdf_path)
...
def test_schema_validation():
    ...
    sample_data = {
        ...
        "clinical_shorthand_detected": [
            {"shorthand": "SOB", "full_term": "Shortness of breath"}
        ]
    }
